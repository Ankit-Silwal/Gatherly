  ...
  io.on('connection', async (socket: Socket) => {
    ...
    socket.on('send_message', async (data: { channelId: string; content: string }) => {
      try {
        ...
        const message = await createMessage(data.channelId, data.content, userId);
        console.log('received_send messgae');
    ...
    socket.on('edit_message', async (data: { messageId: string; content: string }) => {
      try {
        const updated = await editMessage(data.messageId, data.content, userId);
    ...
    socket.on('delete_message', async (data: { messageId: string }) => {
      try {
        const deleted = await deleteMessage(data.messageId, userId);
    ...
    socket.on('mark_read', async (data: { channelId: string; messageId: string }) => {
      await markChannelRead(data.channelId, data.messageId, userId);
    });
